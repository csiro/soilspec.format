...
BrukerOpusBinary <- R6::R6Class("BrukerOpusBinary",
  ...
  public = list(
    ...
    create.standard.meta.data.container = function(meta.list, wavenumbers, opus2, filename) {
      ...
      md[['date_time']] <- as.character(meta.list$date_time_sm) #opus2[[filename]]$basic_metadata$timestamp_string
      ...
      if (status == 0) {
        ...
        warning=function(cond) {
          ...
          if (!is.null(names(spec.data)) && length(names(spec.data)) != 0) {
            ...
            stdmeta <- self$create.standard.meta.data.container(meta.list, wavenumbers, opus2, fname)
